...
<div class="control-group fieldcontain ${hasErrors(bean: drinkLogInstance, field: 'size', 'error')} ">
  ...
  <div class="controls">
    ...
    <g:select id="size" name="size.id" from="${servingSizeList}" optionKey="id" value="${drinkLogInstance?.size?.id ?: servingSizeList.find{ it.name == "12 oz" }?.id }" class="many-to-one" noSelection="['null': '']"/>
